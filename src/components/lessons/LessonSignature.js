...
const content = (
  <div>
    <h2>Signature</h2>
    <p>
      The cryptosatellite can sign messages sent to it by end users. The
      cryptosatellite signs the message, along with the current timestamp using
      its signature key. The current timestamp is appended to preventing
      backdating of signatures. The cryptosat service takes care of
      coordinating the transmission of the message to the appropriate ground
      station when the cryptosatellite is online. To sign a message invoke the
      following API call:
    </p>
    <CodeSnippet code={`promise = cryptosat.sign('hello, world!');`} />
    <p>
      The API call returns a promise which is fulfilled once the satellite
      signed the message and transmitted the signature back to earth. When
      the request has been fulfilled, obtain the result using the await keyword:
    </p>
